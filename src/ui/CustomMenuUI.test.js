...
describe('Custom Menu UI   ', () => {
    ...
    it('should handle showsubmenu', () => {
        ...
        const ui = { '_customStyleName': 'Normal', '_customStyle': { 'styleName': 'Normal', 'mode': 0, 'description': 'Normal', 'styles': { 'align': 'left', 'boldNumbering': true, 'boldSentence': true, 'fontName': 'Tahoma', 'fontSize': '12', 'nextLineStyleName': 'Normal', 'paragraphSpacingAfter': '3', 'toc': false } }, '_popUp': null };
    ...
    it('should handle showsubmenu when popup not null', () => {
        ...
        // custommenuui._stylePopup = {close:()=>{}};
        const ui = { '_customStyleName': 'Normal', '_customStyle': { 'styleName': 'Normal', 'mode': 0, 'description': 'Normal', 'styles': { 'align': 'left', 'boldNumbering': true, 'boldSentence': true, 'fontName': 'Tahoma', 'fontSize': '12', 'nextLineStyleName': 'Normal', 'paragraphSpacingAfter': '3', 'toc': false } }, '_popUp': null };
        expect(custommenuui.showSubMenu(ui, { currentTarget: document.createElement('span') })).toBeUndefined();
    });
    it('should handle removeCustomStyleName', () => {

        expect(custommenuui.removeCustomStyleName(state, 'AFDP_Bullet', null)).toBe(false);
    });
    it('should handle removeCustomStyleName', () => {
        const state = {
            doc: mockdoc,
    ...
    it('should handle showStyleWindow', () => {
        ...
        expect(custommenuui.renameStyleInDocument(statemock, trmock, 'Normal', 'test', {})).toBeUndefined();
        expect(custommenuui.getTheSelectedCustomStyle(statemock)).toBeDefined();

        expect(custommenuui.showAlert()).toBeUndefined();
        const uicommands = { _customStyleName: 'test', _customStyle: { description: 'description', styles: {} } };
